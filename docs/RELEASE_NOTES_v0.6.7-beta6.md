...
- DLSSFG 选择 Reflex 时使用捕获前 Async 基础限帧，独立请求低延迟 On、Boost Off，额外驱动限帧间隔保持 0。Front Edge 兼容选项继续保留。
- 输出队列满时提前反馈背压，修正 FIFO 多次重试的等待统计，并为期限等待接入消息循环计时器。面板分别说明基础限帧、Reflex 状态和 FG 输出方式。
...
- Selecting Reflex with DLSSFG uses Async base pacing before capture and a separate low-latency On, Boost Off request, with zero additional driver interval. The explicit Front Edge compatibility option remains available.
- Applies full-output-queue backpressure before the next base input, accumulates FIFO retry timings, and waits for presentation deadlines through the message loop. The panel separately describes base pacing, Reflex state and FG output.
