...
当前源码、发布工程清单及 0.6.7-local 已部署目录均为 **161 项**，效果器 ID 集合一致，161 份 HLSL 文件 SHA256 全部相同。此清单取代旧版 170 项目录作为本轮 review 的范围。
...
- **DLSS SR → 抗锯齿**：按用户反馈定位为实验性边缘平滑。深度为全零资源、jitter=(0,0)，运动可以选择估算。L／M 的轻微抗锯齿收益来自用户反馈；当前源码仍请求 Preset J，模型选择尚待后续落实。
...
| 用途分类   | 数量 | 子分组                                           |
| ---------- | ----:| ------------------------------------------------ |
| 放大与超分 | 110  | 通用、动画线条、像素画、时序重建、缩小与尺寸整理 |
| 清理修复   | 19   | 降噪、去色带、线条修复                           |
| 抗锯齿     | 13   | 空间、时序                                       |
| 锐化细节   | 6    | 通用锐化                                         |
| 补帧与帧率 | 4    | 补帧、帧率调度                                   |
...
按本项目可实现的实际效果，定位为实验性时序抗锯齿。 用户反馈：缺少重建所需的原始相机抖动条件时，捕获式 DLSS SR 的超分效果不佳；使用 L／M 模型可获得少量抗锯齿效果。当前实现不提供真实深度和相机 jitter：深度资源全零，jitter offset 为 (0,0)，运动向量可以选择估算。

- **用途**：抗锯齿 → 时序。**输出尺寸**：使用效果组配置的输出尺寸。
- **变体差异**：当前源码请求 Balanced / Preset J，尚未提供 L／M 模型选择参数；L／M 的抗锯齿收益属于用户反馈，不能写成当前默认模型的已验证效果。底层仍保留输出尺寸设置。
- **推荐**：进阶／实验。缺少源相机抖动等原生重建条件；L／M 的轻微抗锯齿收益来自用户反馈，当前源码请求的仍是 J。 适用条件／尝试建议：确认实际生效模型和 DLL／驱动支持情况；用静止边缘与运动画面对比抗锯齿收益、重影和开销；后续提供 L／M 入口前，先落实模型选择或覆盖方式。
- **管线位置**：实验性抗锯齿，按实际输入与输出尺寸安排。 建议放在「需要先完成的图像整理（按输入素材选择）」之后。 建议放在「最终锐化／显示风格」之前。 以轻微边缘平滑为尝试目标，先确认模型和输出尺寸，再评价画面收益。
- **条件与取舍**：不提供真实深度或相机 jitter；depth 为全零资源，jitter=(0,0)；允许选择估算运动；当前尺寸检查要求输出每一轴不小于输入；分类为抗锯齿不会将输出尺寸自动锁定为 1×；受支持的 NVIDIA GPU/驱动；包含对应 NGX 功能的构建与运行库。
