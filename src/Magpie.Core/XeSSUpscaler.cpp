...
static bool WaitForD3D12(XeSSUpscaler::Impl& impl) noexcept {
...
static bool WaitForFenceValue(XeSSUpscaler::Impl& impl, uint64_t value) noexcept {
...
XeSSUpscaler::XeSSUpscaler() = default;
...
bool XeSSUpscaler::Resize(
	DeviceResources& deviceResources,
	ID3D11Texture2D* input,
	ID3D11Texture2D* output
) noexcept {
	return Initialize(deviceResources, input, output, _motionRequest);
}

bool XeSSUpscaler::Draw(const NativeEffectDrawContext& drawContext) noexcept {
...
XeSSUpscaler::XeSSUpscaler() = default;
XeSSUpscaler::~XeSSUpscaler() = default;
bool XeSSUpscaler::Initialize(DeviceResources&, ID3D11Texture2D*, ID3D11Texture2D*, MotionVectorRequest) noexcept {
	Logger::Get().Error("XeSS SR support is not enabled in this build");
	return false;
}
bool XeSSUpscaler::Resize(DeviceResources&, ID3D11Texture2D*, ID3D11Texture2D*) noexcept { return false; }
bool XeSSUpscaler::Draw(const NativeEffectDrawContext&) noexcept { return false; }
